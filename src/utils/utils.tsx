...
export function isNum(value: number | null | undefined): boolean {
  return value !== null
    && value !== undefined
    && typeof value === 'number'
}
...
export function useLocalStorageStringState(
  ...
  const setState = useCallback<(newState: string | null) => void>(
    (newState) => {
      ...
      localStorageListeners[key]?.forEach((listener: (arg0: string) => any) =>
        listener(key + '\n' + newState),
      );
    },
    [state, key],
...
export function useEffectAfterTimeout(effect: () => void, timeout: number | undefined) {
  useEffect(() => {
...
export function flatten(obj: { hasOwnProperty: (arg0: any) => any; }, { prefix = '', restrictTo }: any) {
  ...
  (function recurse(obj, current, keys) {
    (keys || Object.keys(obj)).forEach((key: string) => {
      const value = obj[key];
