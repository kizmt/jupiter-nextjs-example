...
export function useMarkGain() {
  ...
  async function getBonfidaPairs() {
    if (!name) {
      return null;
    }
    let data = await BonfidaApi.getPairs();
...
export function useMarkVolume() {
 ...
 (depending on volume of the pair) */
  ...
  /* ==================================================
  try stuff with BonfidaAPI @ansel LOOK! :see_no_evil:*/
  const { name } = useMarket();
...
export function useLastDay() {
  ...
  async function getBonfidaHistory() {
    if (!name) {
      return null;
    }
    return await BonfidaApi.getLastDay(name);
  }

  return useAsyncData(
    getBonfidaHistory,
    tuple('getBonfidaHistory', name),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
    false,
  );
}
...
export function useFillsApi() {
  ...
  async function getBonfidaRecentTrades() {
    if (!market || !wallet) {
      return null;
    }

    const orderAccount = openOrdersAccounts.map(oo => oo.address.toBase58());
    const data = await BonfidaApi.getOwnFills(marketName!, orderAccount);
    return data;
  }

  return useAsyncData(
    getBonfidaRecentTrades,
